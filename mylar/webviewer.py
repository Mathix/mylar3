...
from mylar import logger, db, importer, mb, search, filechecker, helpers, updater, parseit, weeklypull, librarysync, \
    moveit, Failed, readinglist, config
...
class WebViewer(object):
    ...
    def read_comic(self, ish_id=None, page_num=None, size=None):
        ...
        return serve_template(templatename="read.html", pages=image_list, current_page=page_num, np=next_page,
                              pp=prev_page, nop=num_pages, size=user_size_pref, cc=cookie_comic, comicpath=comic_path,
                              ish_id=ish_id)
...
class ComicScanner(object):
    ...
    def user_unpack_comic(self, ish_id, comic_path):
        ...
        if comic_path.endswith(".cbr"):
            ...
            for rar_info in opened_rar.infolist():
                ...
                opened_rar.extract(rar_info, os.path.join(mylar.CONFIG.CACHE_DIR, "webviewer", ish_id))
        elif comic_path.endswith(".cbz"):
            opened_zip = zipfile.ZipFile(comic_path)
            for zip_info in opened_zip.infolist():
                if zip_info.filename[-1] == '/':
                    continue
                zip_info.filename = os.path.basename(zip_info.filename)
                opened_zip.extract(zip_info, os.path.join(mylar.CONFIG.CACHE_DIR, "webviewer", ish_id))
        return
    ...
    def reading_images(self, ish_id):
        ...
        for root, dirs, files in os.walk(image_src):
            for f in files:
                if f.endswith((
                              ".png", ".gif", ".bmp", ".dib", ".jpg", ".jpeg", ".jpe", ".jif", ".jfif", ".jfi", ".tiff",
                              ".tif")):
